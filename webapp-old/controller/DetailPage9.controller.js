sap.ui.define(["sap/ui/core/mvc/Controller",
    ...
    'sap/ui/model/Sorter',
    'sap/ui/model/Filter'
], function (BaseController, MessageBox, Dialog12, Utilities, History, Export, ExportTypeCSV, exportLibrary, Spreadsheet, jszipjs, xlsxjs, Sorter, Filter) {
    "use strict";
    var EdmType = exportLibrary.EdmType;
    var excelImported = "";
    return BaseController.extend("com.sap.build.standard.adminEngine.controller.DetailPage9", {
        ...
        _onOverflowToolbarButtonPress: function (oEvent) {
            ...
            this.mDialog12 = this.mDialog12 || {};
            var oDialog = this.mDialog12[sDialogName];
            if (!oDialog) {
                oDialog = new Dialog12(this.getView());
                this.mDialog12[sDialogName] = oDialog;

                // For navigation.
                oDialog.setRouter(this.oRouter);
            } else {
                this.getView().byId("agrupador").setValue("");
                this.getView().byId("tecnologia").setValue("");
                this.getView().byId("material").setValue("");
                this.getView().byId("descMaterial").setValue("");
            }
        ...
        onDataExport: function (oEvent) {
            var oListBinding = this.getView().getModel().bindList("/Agrupadores", undefined, undefined, undefined, { $select: "*" });
            var oData = [];
            oListBinding.requestContexts().then(function (aContexts) {
                aContexts.forEach(function (oContext) {
                    oData.push(oContext.getObject());
        ...
        onExportOdataV4: function () {
            ...
            if (!this._oTable) {
                this._oTable = this.byId('agrupadorTable');
            }
        ...
        generateTableCsv: function () {
            var that = this;
            var oTable = that.getView().byId("agrupadorTable");
        ...
        _import: function (file) {
            ...
            if (file && window.FileReader) {
                ...
                reader.onload = function (e) {
                    ...
                    var oModelo = new sap.ui.model.json.JSONModel({
                        Agrupadores: excelData
                    });
                    var oModelData = oModelo.getProperty("/");
                    var oColumns = Object.keys(oModelData["Agrupadores"][0]);
                    if (oColumns[0] == "agrupador" &&
                        oColumns[1] == "tecnologia" &&
                        oColumns[2] == "material") {
                        var oGlobalBusyDialog = new sap.m.BusyDialog();
                        oGlobalBusyDialog.open();
                        var oModel = that.getView().getModel();
                        var oListBinding = oModel.bindList("/importAgrupadores", undefined, undefined, undefined, { $$updateGroupId: "agrupadorExcel" });
                            ...
                            if (oEvent.getParameter("success")) {
                                MessageBox.show(
                                    ...
                                    onClose: function (oAction) {
                                        var oListBinding = that.getView().getModel().bindList("/upsert_agrupadores()", undefined, undefined, undefined, undefined);
                                ...
                                that.byId("agrupadorTable").getBinding("items").resetChanges();
                                //   sap.ui.getCore().getMessageManager().removeAllMessages();
                            }
                        }.bind(this));
                        //var  oContext = "";
                        ...
                        $.each(excelData, function (index, value) {
                            var agrupador = value.agrupador.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
                            agrupador = agrupador.replace(/[0-9]/g, '');
                            oData.push({
                        ...
                        if (excelData.length > 0) {
                            //that.onSave();
                            that.getView().getModel().submitBatch("agrupadorExcel");
                        }
                    } else {
                        sap.m.MessageBox.show(
        ...
        onSave: function () {
            ...
            this.getView().getModel().submitBatch("AgrupadoresGroup").then(fnSuccess, fnError);
            //this._bTechnicalErrors = false; // If there were technical errors, a new save resets them.
        },
        onResetChanges: function () {
            this.byId("agrupadorTable").getBinding("items").resetChanges();
        ...
        setHeaderContext: function () {
            ...
            oView.byId("titleCount").setBindingContext(
                oView.byId("agrupadorTable").getBinding("items").getHeaderContext());
        },
        ...
        handleFilterButtonPressed: function () {
            if (!this.DialogFilter) {
                this.DialogFilter = this.DialogFilter = sap.ui.xmlfragment(
                    "DialogFilter",
                    "com.sap.build.standard.adminEngine.view.ViewSettingsDialog5",
                    this
                );
        ...
        handleFilterDialogConfirm: function (oEvent) {
            var oTable = this.byId("agrupadorTable"),
        ...
        handleSortButtonPressed: function () {
            if (!this.DialogSort) {
                this.DialogSort = this.DialogSort = sap.ui.xmlfragment(
                    "DialogSort",
                    "com.sap.build.standard.adminEngine.view.ViewSettingsDialog6",
                    this
                );
                //to get access to the global model
        ...
        onDeleteAll: function () {
            var that = this;
            MessageBox.warning("Tem certeza que seja excluir todos os registros? É recomendado realizar o download dos dados antes de prosseguir", {
                ...
                onClose: function (sAction) {
                    if (sAction == "OK") {
                        ...
                        oGlobalBusyDialog.open();
                        var oModel = that.getView().getModel();
                        var oListBinding = oModel.bindList("/killAgrupadores", undefined, undefined, undefined, { $select: "*" });
