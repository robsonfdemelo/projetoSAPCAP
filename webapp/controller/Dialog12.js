sap.ui.define([
    ...
    return ManagedObject.extend("com.sap.build.standard.adminEngine.controller.Dialog12", {
        constructor: function (oView) {
            this._oView = oView;
            this._oControl = sap.ui.xmlfragment(oView.getId(), "com.sap.build.standard.adminEngine.view.Dialog12", this);
            this._bInit = false;
        },
        ...
        onCreate: function () {
            if (this.getView().byId("agrupador").getValue() == "" || this.getView().byId("tecnologia").getValue() == ""
            || this.getView().byId("material").getValue() == "") {
                ...
                var oList = this.getView().byId("agrupadorTable"),
                ...
                if (oList.getItems()[0]) {
                    ...
                    for (var i = 0; i < oList.getItems()[0].getBindingContext().getBinding().aContexts.length; i++) {
                        ...
                        //sPath = sPath.replace("')", "");
                        var vagrupador = oList.getItems()[i].getBindingContext().getProperty("agrupador");
                        var vtecnologia = oList.getItems()[i].getBindingContext().getProperty("tecnologia");
                        var vmaterial = oList.getItems()[i].getBindingContext().getProperty("material");
                        if (vagrupador == this.getView().byId("agrupador").getValue()
                        && vtecnologia == this.getView().byId("tecnologia").getValue()
                        && vmaterial == this.getView().byId("material").getValue()) {
                            exists = true;
                            break;
                        }
                    }
                ...
                if (exists == false) {
                    var agrupador = this.getView().byId("agrupador").getValue().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
                    agrupador = agrupador.replace(/[^a-zA-Z ]/g, "");
                    var oContext = oBinding.create({
                        "agrupador": agrupador.toUpperCase(),
                        "tecnologia": (this.getView().byId("tecnologia").getValue() + "").toUpperCase() ,
        ...
        _handleValueHelpCloseMateriais: function (evt) {
            ...
            if (oSelectedItem) {
                ...
                that.getView().byId("descMaterial").setValue(oSelectedItem.getDescription());
            }
        },
