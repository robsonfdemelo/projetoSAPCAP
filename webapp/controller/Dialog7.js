sap.ui.define([
    ...
    return ManagedObject.extend("com.sap.build.standard.adminEngine.controller.Dialog7", {
        constructor: function (oView) {
            this._oView = oView;
            this._oControl = sap.ui.xmlfragment(oView.getId(), "com.sap.build.standard.adminEngine.view.Dialog7", this);
            this._bInit = false;
        },
        ...
        onCreate: function () {
            if (this.getView().byId("tipoWo").getValue() == "") {
                sap.m.MessageBox.show(
                    "Preencha o campo Tipo de WO antes de gravar!.",
                    sap.m.MessageBox.Icon.ERROR,
                    "Erro ao atualizar os dados"
                );
            } else {
                var oList = this.getView().byId("tipoWoTable"),
                ...
                if (oList.getItems()[0]) {
                    ...
                    for (var i = 0; i < oList.getItems()[0].getBindingContext().getBinding().aContexts.length; i++) {
                        ...
                        sPath = sPath.replace("')", "");
                        if (sPath == this.getView().byId("tipoWo").getValue()) {
                            exists = true;
                            break;
                        }
                    }
                ...
                if (exists == false) {
                    var oContext = oBinding.create({
                        "tipoWo": this.getView().byId("tipoWo").getValue()
                    });
                    var that = this;
                    ...
                    sap.m.MessageBox.show(
                        "Tipo WO já cadastrado, favor verificar.",
                        sap.m.MessageBox.Icon.ERROR,
                        "Erro ao atualizar os dados"
                    );
                }
