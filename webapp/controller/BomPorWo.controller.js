sap.ui.define(["sap/ui/core/mvc/Controller",
    ...
    return BaseController.extend("com.sap.build.standard.adminEngine.controller.BomPorWo", {
        ...
        onInit: function () {
            ...
            var that = this;
            that.customMaterialFilter = [];
            that.customTipoOSFilter = [];
        ...
        handleFilterDialogConfirm: function (oEvent) {
            ...
            mParams.filterItems.map((oItem) => {
                if(oItem.getKey() !== "codMaterialSAP" && oItem.getKey() !== "tipoOs") {
                    let oObject = oItem.getBindingContext().getObject(),
                    oText = oItem.getText();

                    if(oItem.getKey() == "tipoInstalacao") oText = oText[0]

                    let oFilter = new sap.ui.model.Filter(oItem.getKey(), sap.ui.model.FilterOperator.EQ, oText);
                    aFilters.push(oFilter);
                    console.log(oText)
            ...
            if(this.customMaterialFilter) {
                this.customMaterialFilter.forEach((material) => {
                    var oFilter = new sap.ui.model.Filter("codMaterialSAP", sap.ui.model.FilterOperator.EQ, material);
                    aFilters.push(oFilter);
                });
            }

            if(this.customTipoOSFilter) {
                this.customTipoOSFilter.forEach((tipoOs) => {
                    var oFilter = new sap.ui.model.Filter("idTipoOS", sap.ui.model.FilterOperator.EQ, tipoOs);
                    aFilters.push(oFilter);
                });
            }
        ...
        //Função do filtro
        onSelectionChangeMaterial: function(oEvent){
            var bSelected = oEvent.getParameter('selected');

            if (!this.customMaterialFilter) {
                this.customMaterialFilter = [];
            }
            ...
            if (bSelected) {
                this.customMaterialFilter = [];
                oList.getSelectedContexts().forEach((material) => {
                    this.customMaterialFilter.push(material.getProperty("codMaterialSAP"))
                })
            } else {
                let sValueDel = oEvent.getParameter('listItem').getTitle();
                this.customMaterialFilter = this.customMaterialFilter.filter(element => element !== sValueDel);
                console.log(this.customMaterialFilter)
            }

            if (this.customMaterialFilter.length > 0) {
                var oCustomFilter = sap.ui.core.Fragment.byId("DialogFilter", "customMaterial")
                // @ts-ignore
                oCustomFilter.setFilterCount(this.customMaterialFilter.length);
                // @ts-ignore
                oCustomFilter.setSelected(true);
            } else {
                var oCustomFilter = sap.ui.core.Fragment.byId("DialogFilter", "customMaterial")
                console.log(this.customMaterialFilter.length)
                oCustomFilter.setFilterCount(this.customMaterialFilter.length)
            }

        },

        onSelectionChangeTipoOS: function(oEvent){
            var bSelected = oEvent.getParameter('selected');

            if (!this.customTipoOSFilter) {
                this.customTipoOSFilter = [];
            }
            ...
            if (bSelected) {
                this.customTipoOSFilter = [];
                oList.getSelectedContexts().forEach((tipoOs) => {
                    this.customTipoOSFilter.push(tipoOs.getProperty("tipoOs"))
                })
            } else {
                let sValueDel = oEvent.getParameter('listItem').getTitle();
                this.customTipoOSFilter = this.customTipoOSFilter.filter(element => element !== sValueDel);
            }

            if (this.customTipoOSFilter.length > 0) {
                var oCustomFilter = sap.ui.core.Fragment.byId("DialogFilter", "customTipoOS")
                // @ts-ignore
                oCustomFilter.setFilterCount(this.customTipoOSFilter.length);
                // @ts-ignore
                oCustomFilter.setSelected(true);
            } else {
                var oCustomFilter = sap.ui.core.Fragment.byId("DialogFilter", "customTipoOS")
                console.log(this.customTipoOSFilter.length)
                oCustomFilter.setFilterCount(this.customTipoOSFilter.length)
            }
            console.log(customTipoOSFilter)
        ...
        onSearchMaterial: function(oEvent){
            ...
            if (sQuery && sQuery.length > 1) {
                var filter = new sap.ui.model.Filter("codMaterialSAP", sap.ui.model.FilterOperator.Contains, sQuery);
                aFilters.push(filter);

                // update list binding
                // @ts-ignore
                var oBinding = sap.ui.core.Fragment.byId("DialogFilter", "MaterialCustomList").getBinding("items")
                oBinding.sOperationMode = sap.ui.model.odata.OperationMode.Server;
                oBinding.filter(aFilters);
        ...
        onSearchTipoOS: function(oEvent){
            var aFilters = [];
            var sQuery = oEvent.getSource().getValue();
            if (sQuery && sQuery.length > 1) {
                var filter = new sap.ui.model.Filter("tipoOs", sap.ui.model.FilterOperator.Contains, sQuery);
                aFilters.push(filter);

                // update list binding
                // @ts-ignore
                var oBinding = sap.ui.core.Fragment.byId("DialogFilter", "TipoOSCustomList").getBinding("items")
                oBinding.sOperationMode = sap.ui.model.odata.OperationMode.Server;
                oBinding.filter(aFilters);
            } else if (sQuery.length == 0) {
                // @ts-ignore
        ...
        //Reset Filter *arrumar*
        onResetFilters: function (oEvent){

            // @ts-ignore
            sap.ui.core.Fragment.byId("DialogFilter", "MaterialCustomList").removeSelections();
            this.customMaterialFilter = [];
            sap.ui.core.Fragment.byId("DialogFilter", "TipoOSCustomList").removeSelections();
            this.customTipoOSFilter = [];


            var oCustomMaterial = sap.ui.core.Fragment.byId("DialogFilter", "customMaterial");
            var oCustomTipoOS = sap.ui.core.Fragment.byId("DialogFilter", "customTipoOS");

            // @ts-ignore
            oCustomMaterial.setFilterCount(0);
            // @ts-ignore
            oCustomMaterial.setSelected(false);
            // @ts-ignore
            oCustomTipoOS.setFilterCount(0);
            // @ts-ignore
            oCustomTipoOS.setSelected(false)

        },

        formatarTipoInstalacao: function (inst){
            var tipoInst
            switch(inst){
                case 'C':
                    tipoInst = "Casa"
                ...
                case 'R':
                    ...
                    break
                case 'E' :
                    tipoInst = "Entrada de Coletivos"
                    break
                default :
                    tipoInst = "None"
            }
            return `${inst} : ${tipoInst}`
        }
