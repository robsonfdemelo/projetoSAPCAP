        ...
        _import: function (file) {
            ...
            if (file && window.FileReader) {
                ...
                reader.onload = function (e) {
                    ...
                    var oModelo = new sap.ui.model.json.JSONModel({
                        MateriaisExcecao: excelData
                    });
                    var oModelData = oModelo.getProperty("/");
                    var oColumns = Object.keys(oModelData["MateriaisExcecao"][0]);
                    if (oColumns[0] == "material") {
                        var oGlobalBusyDialog = new sap.m.BusyDialog();
                        oGlobalBusyDialog.open();
                        var oModel = that.getView().getModel();
                        var oListBinding = oModel.bindList("/importMateriaisExcecao", undefined, undefined, undefined, { $$updateGroupId: "tipoMaterial" });
                        ...
                        var oContext = oListBinding.create({
                            "MateriaisExcecao": oData
                        }, true);

                        if (excelData.length > 0) {
                            //that.onSave();
                            that.getView().getModel().submitBatch("tipoMaterial");
                        }
                    } else {
                        sap.m.MessageBox.show(
